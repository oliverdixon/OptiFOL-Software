...
std::size_t Substitution::KeyRefHashingFunctor::operator()(const std::reference_wrapper<const Variable> object)
        const noexcept
{
...
bool Substitution::KeyRefEqualityFunctor::operator()(
        const std::reference_wrapper<const Variable> lhs_object,
        const std::reference_wrapper<const Variable> rhs_object) const noexcept
{
    return lhs_object.get().hash() == rhs_object.get().hash();
}

Substitution::Substitution(std::initializer_list<std::pair<const Variable &, const IProcessedTerm &>> entries)
{
