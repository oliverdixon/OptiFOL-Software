...
#include "../Mutable/Terms/MutableVariableNode.hpp"

namespace optifol
{

std::size_t Substitution::KeyRefHashingFunctor::operator()(const std::reference_wrapper<const MutableVariableNode> object)
        const noexcept
{
...
bool Substitution::KeyRefEqualityFunctor::operator()(
        const std::reference_wrapper<const MutableVariableNode> lhs_object,
        const std::reference_wrapper<const MutableVariableNode> rhs_object) const noexcept
{
    return lhs_object.get().hash() == rhs_object.get().hash();
}

Substitution::Substitution(std::initializer_list<std::pair<const MutableVariableNode &, const IMutableTermNode &>> entries)
{
