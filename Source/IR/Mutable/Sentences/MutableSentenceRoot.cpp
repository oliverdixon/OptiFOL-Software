...
MutableSentenceRoot::MutableSentenceRoot(std::unique_ptr<IMutableSentenceNode> &&sentence) :
    sentence(std::move(sentence))
{ }

std::unique_ptr<IMutableSentenceNode> MutableSentenceRoot::clone() const
{
    return std::make_unique<MutableSentenceRoot>(sentence->clone());
}

void MutableSentenceRoot::flip_polarity()
{
    is_positive = !is_positive;
}

bool MutableSentenceRoot::is_negative_polarity() const
{
    return !is_positive;
}

std::unique_ptr<IMutableSentenceNode> MutableSentenceRoot::take_sentence()
{
    return std::move(sentence);
}

const IMutableSentenceNode * MutableSentenceRoot::observe_sentence() const
{
    return sentence.get();
}

void MutableSentenceRoot::put_sentence(std::unique_ptr<IMutableSentenceNode> &&sentence)
{
