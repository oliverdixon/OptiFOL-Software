...
/**
 * @class TestResult
 * @brief Describes a single result provided by a software unit-testing framework. The TestResult is agnostic to the
 *  framework, but does encode the test name, test fixture name, and executable path for which the result is relevant.
 */
class TestResult :
        public IHashable
{
public:
    /**
     * @class Partial
     * @brief Describes a partial result, or additional message produced by the testing framework. Partial results exist
     *  typically, though not exclusively, in the case of a failed test result. A single result may contain multiple
     *  partial results.
     */
    struct Partial
    {
     ...
     * @details Note that the test fixture name is not specified upon construction. This is due to a requirement on many
     *  backend parsers that the fixture object is parsed after its constituent test results. Therefore the fixture name
     *  is specified after construction with @ref populate_test_fixture_name.
     ...
     * @warning This hash function is incomplete. It will only uniquely identify the Test with which the result is
     *  associated, according to the test name and fixture name. It does not consider the fabric of the result.
     */
