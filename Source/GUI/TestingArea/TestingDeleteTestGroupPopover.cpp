...
TestingDeleteTestGroupPopover::TestingDeleteTestGroupPopover(Gtk::Builder &builder, TestingArea &testing_area) :
    ...
    test_group_entry(GTKHelpers::get_widget<Gtk::Entry>(popover_name, builder, "delete_test_group_name"))
{
    ...
    const auto confirm_button = GTKHelpers::get_widget<Gtk::Button>(popover_name, builder, "delete_test_group_confirm");
    const auto cancel_button = GTKHelpers::get_widget<Gtk::Button>(popover_name, builder, "delete_test_group_cancel");

    // Set up buttons and self.
    confirm_button->signal_clicked().connect(sigc::mem_fun(*this, &TestingDeleteTestGroupPopover::confirm_button_clicked));
    cancel_button->signal_clicked().connect(sigc::mem_fun(*this, &TestingDeleteTestGroupPopover::cancel_button_clicked));
...
void TestingDeleteTestGroupPopover::confirm_button_clicked() const noexcept
{
    try {
        ...
        /*
         * Linear search is acceptable; we maintain a relatively small number of test groups, and it's not worth the
         * additional overhead of maintaining an index hash table.
         */
        for (guint group_index = 0; group_index < group_count; ++group_index)
            if (groups->get_item(group_index) == target_group) {
                ...
                popover_logger->debug("Removed Test Group \"" + target_group->property_name().get_value() + "\".");
                break;
            }
