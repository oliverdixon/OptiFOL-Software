...
const log4cxx::LoggerPtr TestingFailedView::area_logger = Logging::get_logger({"GUI", "TestingCompliance",
    "FailedTests"});
...
void TestingFailedView::select_model(const Glib::RefPtr<Subsystem> &new_subsystem)
{
    ...
    tree_model = Gtk::TreeListModel::create(active_subsystem->get_test_groups(),
        &ITestModelNode::get_given_results_tree, true, true);
        ...
        if ((column = columns->get_typed_object<Gtk::ColumnViewColumn>(position)) != nullptr) {
            ...
            if (gtk_id == "failed_test_message") {
                ...
                factory->signal_bind().connect([](const Glib::RefPtr<Gtk::ListItem> & list_item) noexcept -> void
                {
                    ...
                    const auto typed_result = dynamic_cast<const PartialTestResult *>(list_item->get_item().get());

                    if (label == nullptr || typed_result == nullptr)
                        return;

                    Glib::Binding::bind_property(
                        typed_result->property_file(),
        ...
        const guint initial_index, const guint removed_count, const guint added_count) const noexcept
{
    ...
    /*
     * If the tree model doesn't exist, we always want to disable the failed view: no model means no failed tests.
     *
     * Recall that the number of items in the failed model is lower-bounded by the number of test groups. Once the
     * number of items in the failed model reaches the number of test groups, we know that all test groups must be
     * empty. Since they are empty, there are no failure records, thus the container should be hidden.
     */
