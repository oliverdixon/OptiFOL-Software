...
{
    /*
     * Establish the mapping between popover-raising actions and corresponding MenuButtons. If a popover constructor has
     * been defined, call that immediately prior to the popup call.
     */
    for (const auto& [name, menu_button, popover, enabled] : map) {
        const auto action_source = action_group->add_action(name, [popover]
        {
            popover->popup();
        });
    ...
    /*
     * Set up a callback for the changing state of actions. When the state of an underlying Gio::SimpleAction is
     * modified, this callback is triggered to locate the corresponding Gtk::MenuButton and toggle its sensitivity
     * accordingly. Therefore, the graphical button state always matches the availability of the action.
     */
    action_group->signal_action_enabled_changed().connect([this](const Glib::ustring& action_name, const bool enabled)
    {
...
{
    const auto action = std::dynamic_pointer_cast<Gio::SimpleAction>(
