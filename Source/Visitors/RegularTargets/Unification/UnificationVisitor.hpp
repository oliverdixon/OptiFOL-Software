 ...
 *  </p>
 *  <p>
 *      The most basic UnificationVisitor is unidirectional; that is, it accepts a "generalisation" and an "instance",
 *      and substitutions may only be made in the generalisation. If substitutions should also be made in the instance,
 *      use the BidirectionalUnificationVisitor instead, which includes an occurs-check to verify correctness of
 *      substitutions.
 *  </p>
 *  <p>
 ...
 *      </ul>
 *      These rules are reflected in the visitor member functions, and must be respected by unifiable types implementing
 *      the acceptor member functions. Note that triple-despatch may be necessary in some cases (acceptor calling
 *      another overload acceptor, which then calls the visitor) to correctly explore term-specialised cases.
 *  </p>
 *  <p>
 ...
 *  </p>
 * @see BidirectionalUnificationVisitor for the unification visitor to produce substitutions on both the generalisation
 *  and the instance.
...
public:
     ...
     *  number of term arguments, have the same polarity, and the terms can be zipped and mutually unified i.a.w. other
     ...
     * @param variable_inst The RHS Variable to unify
     * @return False, since a Function and a Variable could only be unified if the Variable could be substituted. In
...
protected:
     ...
     * @details To unify a variable with a generic term in the unidirectional case, the mapping must either be new, or
     *  be identical to an existing entry.
     ...
     */
    [[nodiscard]] virtual bool variable_generic(const Variable &variable_gen, const IProcessedTerm &generic_term_inst);
