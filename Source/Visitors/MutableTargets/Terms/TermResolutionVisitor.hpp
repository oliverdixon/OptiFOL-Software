 ...
 * @brief The TermResolutionVisitor rewrites a term-based model according to a variable set of substitution rules.
 */
...
public:
     ...
     * @param rewriting_rules_hook A weak reference, guaranteed to persist for the lifetime of the TermResolutionVisitor
     *  object, mapping term names to replacement terms. Keys are views and have the same lifetime guarantees as the
     *  map.
     */
     ...
     * @details Given an @f$ N @f$-ary function @f$ f \left( x_1, \ldots, x_N \right) @f$, the visitor rewrites the
     *  model to produce @f$ f^\prime \left( x_1^\prime, \ldots, x_N^\prime \right) @f$ over the ruleset @f$ V @f$ where
     *  @f[
     *      \alpha =
     *      \begin{cases}
